...
export default function UserUrl() {
  ...
  return (
    <>
      ...
      <section className="flex-1 overflow-hidden rounded-2xl border border-white/10 bg-white/[0.02]">
        ...
        {!isLoading && !isError && links.length > 0 ? (
          <div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b border-white/10 text-left text-[10px] font-semibold uppercase tracking-wider text-white/35"><th className="px-6 py-3">Short URL</th><th className="hidden px-6 py-3 sm:table-cell">Original URL</th><th className="px-6 py-3 text-center">Clicks</th><th className="px-6 py-3 text-right">Copy</th></tr></thead><tbody>
            {links.map((link) => {
              const shortUrl = `${window.location.protocol}//${window.location.hostname}:3000/${link.short_url}`;
              return <tr className="border-b border-white/5 hover:bg-white/[0.04]" key={link._id}><td className="px-6 py-4"><a className="font-semibold text-nyc-yellow hover:underline" href={shortUrl} rel="noreferrer" target="_blank">/{link.short_url}</a></td><td className="hidden px-6 py-4 text-white/45 sm:table-cell" title={link.full_url}>{truncate(link.full_url, 48)}</td><td className="px-6 py-4 text-center">{link.clicks ?? 0}</td><td className="px-6 py-4 text-right"><button className="rounded-md border border-white/15 px-3 py-1.5 text-xs font-medium text-white/60 hover:border-nyc-yellow hover:text-nyc-yellow" onClick={() => handleCopy(shortUrl, link._id)} type="button">{copiedId === link._id ? "Copied" : "Copy"}</button></td></tr>;
            })}
          </tbody></table></div>
        ) : null}
      </section>
